...
def load_checkpoint(filepath):
    ...
    model.classifier = nn.Sequential(OrderedDict([
                          ('fc1', nn.Linear(25088, 500)),
                          ('relu', nn.ReLU()),
                          ('fc2', nn.Linear(500, len(checkpoint['class_to_idx']))),
                          ('output', nn.LogSoftmax(dim=1))
                          ]))
...
loadedModel = load_checkpoint('checkpoint.pth')
print(loadedModel)
...
def predict(image_path, model, topk=5):
    ...
    for targetIndex in top_class:
        ...
        for classNumber, index in loadedModel.class_to_idx.items():
...
probs, classes = predict(imagePath, loadedModel)
classNames = []
for item in classes:
    classNames.append(cat_to_name[item])


ax = imshow(process_image(imagePath), title = classNames[0])


fig, ax = plt.subplots()
ax.barh(classNames, probs, align='center',color='#1f77b4', ecolor='black')
